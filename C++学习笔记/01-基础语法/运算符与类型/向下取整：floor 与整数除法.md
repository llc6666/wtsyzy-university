...
输出 `std::floor(-1.2)`、`7 / 3` 与 `-7 / 3`。前者应为 `-2`，第二项为 `2`，第三项在 C++ 中按零截断为 `-2`，不是数学上的向下取整 `-3`。这组输入能区分三种经常混淆的行为。

## 边界

整数除法只在非负数场景下可直接当作向下取整；参与运算的两个操作数都是整数时，结果已经截断，之后再调用 `std::floor` 无法恢复小数部分。
